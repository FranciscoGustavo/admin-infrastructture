...
class NewOrder extends Component {
    constructor(props){
        ...
        this.props.dispatch(actions.getOrder(id, this.props.user.jwt));
    ...
    name(){
        if(this.props.order.client) return this.props.order.client[0].name
        return "Selecionar Cliente"
    }

    id(){
        if(this.props.order.client) return this.props.order.client[0]._id
        return false
    }
...
function mapStateToProps(state, ownProps){
    return { user : state.users, clients : state.clients, products : state.search, order : state.orders  }
}
